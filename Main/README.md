...
- `Library`: searchable transcript library with topic/tag multi-select filters, source links, preview/copy, sidecar downloads, index repair, Study Guide generation, and topic classification.
...
- Saved model profiles have a `Test` action that checks the configured OpenAI-compatible endpoint without sending transcript text.
...
- Repairs `library.json` from transcript Markdown files if the index drifts after manual file edits.
...
- Use `Test` after saving a model profile to confirm chat completions respond before sending any transcript excerpts to it.
...
- `Repair Index` scans the active output folder and rebuilds `library.json` from transcript-like Markdown files. It does not move or edit transcript files.
...
Do not open `index.html` directly. Start the local Python server and open `http://127.0.0.1:8765` or use `start-tool.bat`.

### No captions found

Click `Check Captions` to inspect available tracks. Some videos do not expose subtitles or automatic captions, and private or restricted videos are not supported by this caption-only version.

### Rate limit or sudden YouTube failures

YouTube extractor behavior can change. If videos that used to work suddenly fail, update `yt-dlp`, retry later, and check the Settings diagnostics panel for the local caption engine status.

### PO Tokens, cookies, and account access

The app does not use YouTube cookies, login, or PO Tokens by default. If those capabilities are ever added, they should stay explicit opt-in because they can involve private account data.

### Output folder confusion

The default output folder is `outputs` inside `Main`. If `Output Folder` is changed in Settings, new transcripts and the active `library.json` are written to that configured folder. Existing transcripts are not moved automatically.

### Local model profile issues

Local AI models need a running OpenAI-compatible server. For Ollama, LM Studio, vLLM, or similar tools, confirm the server is running, the base URL includes the API path such as `/v1`, and the model name is available on that server.

For LM Studio, the usual base URL is `http://127.0.0.1:1234/v1` after the local server is started. Small local models may need compact prompts, while larger-context local models can accept much more transcript text. Each model profile can set Study Guide source count, input character budget and output token budget.

### Port already in use

If port `8765` is busy, close the current TubeScribe Local server/window first. A stale `server.pid` file is runtime state and is not the source of truth; the active listener on the port matters.
